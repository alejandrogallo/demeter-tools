...
def get_centers(affiliation):
    regexes = [
        re.compile(r"universi[dt].[dt]\s+([^;,.]+)", re.I),
        re.compile(r"universit.\s+de\s+([^;,.]+)", re.I),
        re.compile(r"([^\s]+)\s+universi[dt].[dt]\s+([^;,.]+)", re.I),
    ]
    for regex in regexes:
        m = regex.findall(affiliation)
        if m:
            print(m)
            return ('university', m)
    return False
